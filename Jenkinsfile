pipeline {
    ...
    environment {
        DOCKER_IMAGE = "nexus.blueappleonline.com:5001/django-app-deploy:${BUILD_NUMBER}"
        BRANCH_NAME = 'dev'
    }
        ...
        stage('Push Docker Image') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
                    sh '''
                    echo "$DOCKER_PASS" | docker login nexus.blueappleonline.com:5001 -u "$DOCKER_USER" --password-stdin
